...
double newton(double* z, double x) 
{
	double sum = z[0];
	int n = 6;
	double x0 = 1, h = 1;
...
double newton2(double* z, double x)
{
	double sum = z[6];
	int n = 6;
	double x0 = 7, h = 1;
...
int main()
{
	...
	{
		cout << "f(" << x << ") = " << my_lagrange(a, b, x, x_arr, z) << "\n";
		fout << x << " " << my_lagrange(a, b, x, x_arr, z) << "\n";
	}
	...
	file2.open("sp.txt");
	int s, month = c;
	cout << "enter polinom degree (up to 5): ";
	cin >> s;

	vector<double>  temperature;
	vector<int> stroki;
	int nstrok = 0;
	for (int i = 0; i < 150; i++) if (temp[i][month] != 999.9)
	{ temperature.push_back(temp[i][month]); 
	nstrok++;
	stroki.push_back(nstrok);
	}
	cout << "quantiti of correct temperature: " << temperature.size() << "\n";
	int n1 = stroki.size();
